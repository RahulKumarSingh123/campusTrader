...
export default function AddListing(){
    ...
    async function setDetails(){
        const {data}=await axios.get(`http://localhost:4000/listing/get/${id}`);
        const details=data?.data;
        console.log(details);
        titleRef.current.value=details.itemName;
        descriptionRef.current.value=details.description;
        categoryRef.current.value=details.category;
        statusRef.current.value=details.status;
        priceRef.current.value=details.price;
        conditionRef.current.value=details.condition;
    }
    ...
    async function editListing(e){
        e.preventDefault();
        const response=await axios.put(`http://localhost:4000/listing/update/${id}`,{
            itemName:titleRef.current.value,
            description:descriptionRef.current.value,
            category:categoryRef.current.value,
            condition:conditionRef.current.value,
    ...
    return(
        <div className="bg-gray-200 flex items-center justify-center min-h-[600px]">
            <form method="post" className="px-11 py-11 bg-white rounded-2xl w-1/3 flex h-full flex-col items-center gap-2" onSubmit={(id=="new")?addListing:editListing}>
                <h3 className="font-bold text-xl text-center ">{(id=="new")?"Add":"Edit"} Product</h3>
                ...
                {id=="new" && <input ref={imageRef} className="px-5 py-1.5 rounded-lg w-full bg-gray-100" type="file"/>}
                <button className="px-2 py-1.5 rounded-lg w-full bg-amber-600 font-medium text-lg mt-2 text-white" type="submit">{(id=="new")?"Add":"Edit"} Product</button>
            </form>
        </div>
    )
}